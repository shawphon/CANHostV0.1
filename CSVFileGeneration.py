...
def OutNameandDes(lines_search, pos_Name, pos_attribute):
    """在列表中寻找帧ID及其某个属性，并以字典形式返回 """
    ...
    for line in lines_search:
        string = line.split('\"')
        str = string[pos_Name].split()
        idandattridic[str[3]] = string[pos_attribute]

    return idandattridic

def OutSignalNameandDescription(dbclines):
    """output the message information in the format of dic"""

    sgNameandDesDic = {}
    header_string = 'CM_'
    BO_lines = Search_Str(dbclines, header_string, condi_code='SG_')
    sgNameandDesDic = OutNameandDes(BO_lines, 0, 1)

    return sgNameandDesDic
...
def Create_csv(listInfo, mode, dirPath):
    ...
    with open(path, mode, newline='') as f:
        ...
        csv_head = [listInfo[0], listInfo[1], listInfo[2], listInfo[3], listInfo[4]]
...
def CreatCSVFile(dirPath, fileName):
    ...
    # print(fileName)
    listColumn = ["MessageID", "SignalName", "Period", "GroupName", "Description"]
    listInfo = []
    Create_csv(listColumn, 'w', dirPath)  # 添加列表列名
    lines = Read_File(fileName)

    dicNameandDes = OutSignalNameandDescription(lines)
        ...
        if lines[firstBO].strip().split()[-1] == PCU:
            ...
            for j in range(firstBO, secondBO):
                if "SG_" in lines[j]:
                    ...
                    signalName = lines[j].strip().split()[1]
                    listInfo.append(lines[j].strip().split()[1])  # 为csv文件添加SignalName
                    listInfo.append(period)  # 为csv文件添加Period
                    listInfo.append("Received")
                    if signalName in dicNameandDes.keys():
                        listInfo.append(dicNameandDes[signalName])
                    else:
                        listInfo.append(signalName)
